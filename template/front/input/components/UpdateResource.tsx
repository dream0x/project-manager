...
}) satisfies z.ZodType<UpdateResourceRequest>;

// 作成フォーム
export default function UpdateResource(props: { resource: Resource }) {
	// 作成リクエスト
	const updateResource = useResourceStore((state) => state.update);

	// フォームのデフォルト値
	const updateResourceDefaultValues: UpdateResourceRequest = {
	...
	const { control, handleSubmit, formState, reset } =
		useForm<UpdateResourceRequest>({
			...
			defaultValues: updateResourceDefaultValues,
		});

	// Resourceの更新に合わせてフォームのデフォルト値を同期
	useEffect(() => {
		reset({
			name: props.resource.name
		});
	}, [reset, props.resource.name]);

	// フォーム送信時処理
	const submit = async (values: UpdateResourceRequest) => {
		updateResource(props.resource.id, values);
		reset();
	};

	const deleteResource = useResourceStore((state) => state.delete);
	const deleteSubmit = () => {
		...
		<CommonDialog
			...
			content={(closeDialog) => (
				<FormArea
					...
					submit={async (values: UpdateResourceRequest) => {
						await submit(values);
						closeDialog();
					}}
					formState={formState}
